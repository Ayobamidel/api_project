...
# Northwind API request for Employees
    url =  'https://demodata.grapecity.com/northwind/api/v1/Employees'
    response = requests.get(url)
    data = response.json()

    # Load data into postgres
    df = pd.json_normalize(data)
    df.to_sql('employees_raw', engine, if_exists = 'replace', index= False)
    engine.dispose()
